(() => {
    ...
    if (newSlider) new swiper_core_Swiper(".new-swiper", {
        modules: [ Navigation, Pagination ],
        observer: true,
        observeParents: true,
        slidesPerView: 4,
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        const radioButtons = document.querySelectorAll(".collections__input");
        const showMoreButton = document.querySelector("[data-showmore-button]");
        radioButtons.forEach((function(radioButton) {
            radioButton.addEventListener("change", (function() {
                if (this.checked) {
                    showMoreButton.textContent = this.id === "7days" ? "See All Collection" : "Close";
                    showMoreButton.click();
                }
            }));
        }));
        showMoreButton.addEventListener("click", (function() {
            console.log("Клікнуто на кнопку");
        }));
    }));
    window.addEventListener("resize", (function() {
        var detailsElements = document.querySelectorAll("details");
        if (window.innerWidth <= 991) detailsElements.forEach((function(detail) {
            if (!detail.hasAttribute("close")) detail.open = false;
        })); else detailsElements.forEach((function(detail) {
            if (!detail.hasAttribute("open")) detail.open = true;
        }));
    }));
    window["FLS"] = true;
    isWebp();
